...
const scssTask = function(fileName, path) {
  ...
  return sass(path + fileName + '.scss', { style: 'expanded', sourcemap: false, noCache: true })
    ...
    .pipe(header(headerComment, { pkg, fileName(file) {
    ...
    .pipe(minifycss())
    .pipe(gulp.dest(distPath))
    .pipe(header(headerMiniComment, { pkg, fileName(file) {
      const name = file.path.split(file.base);
      return name[1].replace('\\', '');
    } }))
    .pipe(gulp.dest(distPath))
...
gulp.task('js', function() {
  return gulp.src('./src/editormd.js')
    ...
    .pipe(gulp.dest('./'))
    .pipe(header(headerMiniComment, { pkg, fileName(file) {
      const name = file.path.split(file.base + ((os.platform() === 'win32') ? '\\' : '/'));
      return name[1].replace(/[\\\/]?/, '');
    } }))
    .pipe(gulp.dest('./'))
...
gulp.task('cm-mode', function() {
  ...
  for (const i in codeMirror.modes) {
    const mode = codeMirror.modes[i];
...
gulp.task('cm-addon', function() {
  ...
  for (const i in codeMirror.addons) {
    const addon = codeMirror.addons[i];
