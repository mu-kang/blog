...
(function(mod) {
  ...
  function setDocumentHeight(cm, measure) {
    ...
    const total = measure.docHeight + cm.display.barHeight;
  ...
  function updateWidgetHeight(line) {
    if (line.widgets) {
      for (let i = 0; i < line.widgets.length; ++i) { line.widgets[i].height = line.widgets[i].node.offsetHeight; }
    }
  }
  ...
  function applyTextInput(cm, inserted, deleted, sel) {
    ...
    let textLines = splitLines(inserted),
      multiPaste = null;
    ...
    for (let i = sel.ranges.length - 1; i >= 0; i--) {
      ...
      const changeEvent = { from, to, text: multiPaste ? multiPaste[i % multiPaste.length] : textLines,
    ...
    onContextMenu(e) {
      ...
      function prepareSelectAllHack() {
        if (te.selectionStart != null) {
          ...
          const extval = te.value = '\u200b' + (selected ? te.value : '');
  ...
  ContentEditableInput.prototype = copyObj({
    init(display) {
      ...
      on(div, 'compositionstart', function(e) {
        const data = e.data;
        input.composing = { sel: cm.doc.sel, data, startData: data };
        if (!data) return;
    ...
    pollContent() {
      ...
      const newText = splitLines(domTextBetween(cm, fromNode, toNode, fromLine, toLine));
  ...
  function posToDOM(cm, pos) {
    ...
    const result = nodeAndOffsetInLineMap(info.map, pos.ch, 'left');
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    let bidi = getOrder(lineObj),
      dist = lineObj.text.length;
    let from = lineLeft(lineObj),
  ...
  function buildToken(builder, text, style, startStyle, endStyle, title, css) {
    if (!text) return;
    let special = builder.cm.options.specialChars,
      mustWrap = false;
  ...
  function moveVisually(line, start, dir, byUnit) {
    ...
    let pos = getBidiPartAt(bidi, start),
      part = bidi[pos];
