...
(function(mod) {
  ...
  */
  ...
  function svxGenIndent(stream, state) {
    ...
    const statementIndentUnit = svxindentUnit;
  ...
  CodeMirror.defineMIME('text/x-svx', {
    ...
    hooks: {
      ...
      tokenBase(stream, state) {
        let vxIndent = 0,
          style = false;
        const svxisOperatorChar = /[\[\]=:]/;
        const svxkpScopePrefixs = {
          '**': 'variable-2', '*': 'variable-2', $$: 'variable', $: 'variable',
          '^^': 'attribute', '^': 'attribute' };
        const ch = stream.peek();
        const vxCurCtlFlowCharValueAtStart = state.svxCurCtlFlowChar;
        if (state.vxCodeActive == true) {
          if (/[\[\]{}\(\);\:]/.test(ch)) {
