  ...
  CodeMirror.defineMode('stylus', function(config) {
    ...
    const tokenBase = function(stream, state) {
      ...
      if (stream.match(/^\$?[a-z][\w-]+\s?=(\s|[\w-'"\$])/i)) {
        stream.backUp(2);
        const cssPropertie = stream.current().toLowerCase().match(/[\w-]+/)[0];
        return cssProperties[cssPropertie] === undefined ? 'variable-2' : 'property';
      } else if (stream.match(/\$[\w-\.]+/i)) {
        return 'variable-2';
      } else if (stream.match(/\$?[\w-]+\.[\w-]+/i)) {
        const cssTypeSelector = stream.current().toLowerCase().match(/[\w]+/)[0];
    ...
    const tokenLexer = function(stream, state) {
    ...
    function buildStringTokenizer(quote, greedy) {
      ...
      function stringTokenizer(stream, state) {
        ...
        const endingString = ((nextChar !== '\\' && peekChar === quote) ||
