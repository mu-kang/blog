  ...
  CodeMirror.defineMode('q', function(config) {
    ...
    function tokenBase(stream, state) {
      ...
      if (sol) {
        if (c == '/') { return (state.tokenize = tokenLineComment)(stream, state); } else if (c == '\\') {
          if (stream.eol() || /\s/.test(stream.peek())) { return stream.skipToEnd(), /^\\\s*$/.test(stream.current()) ? (state.tokenize = tokenCommentToEOF)(stream, state) : state.tokenize = tokenBase, 'comment'; }
    ...
    return {
      ...
      indent(state, textAfter) {
        ...
        if (!context) { return 0; } else if (context.type == 'pattern') { return context.col; } else if (context.align) { return context.col + (closing ? 0 : 1); }
