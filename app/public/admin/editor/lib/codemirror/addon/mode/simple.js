...
(function(mod) {
  ...
  function asToken(val) {
    ...
    const result = [];
    for (let i = 0; i < val.length; i++) { result.push(val[i] && val[i].replace(/\./g, ' ')); }
    return result;
  }
  ...
  function enterLocalMode(config, state, spec, token) {
    let pers;
    if (spec.persistent) {
      for (let p = state.persistentStates; p && !pers; p = p.next) { if (spec.spec ? cmp(spec.spec, p.spec) : spec.mode == p.mode) pers = p; }
    }
    ...
    if (spec.persistent && !pers) { state.persistentStates = { mode, spec: spec.spec, state: lState, next: state.persistentStates }; }
