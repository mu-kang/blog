...
(function(mod) {
  ...
  function wrapRange(cm, from, to, options) {
    ...
    const column = options.column || 80;
    ...
    for (let i = 0; i < lines.length; ++i) {
      ...
      while (curLine.length > column) {
        const bp = findBreakPoint(curLine, column, wrapOn, killTrailing);
  ...
  CodeMirror.defineExtension('wrapParagraphsInRange', function(from, to, options) {
    ...
    if (paras.length) {
      cm.operation(function() {
        for (let i = paras.length - 1; i >= 0; --i) { madeChange = madeChange || wrapRange(cm, Pos(paras[i].from, 0), Pos(paras[i].to - 1), options); }
      });
    }
    return madeChange;
  });
