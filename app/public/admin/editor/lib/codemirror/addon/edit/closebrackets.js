...
(function(mod) {
  ...
  const DEFAULT_TRIPLES = "'\"";
  const DEFAULT_EXPLODE_ON_ENTER = '[]{}';
  ...
  CodeMirror.defineOption('autoCloseBrackets', false, function(cm, val, old) {
    ...
    const map = buildKeymap(pairs, triples);
  ...
  function enteringString(cm, pos, ch) {
    ...
    const stream = new CodeMirror.StringStream(line.slice(0, pos.ch) + ch + line.slice(pos.ch), 4);
    stream.pos = stream.start = token.start;
    for (;;) {
      const type1 = cm.getMode().token(stream, token.state);
