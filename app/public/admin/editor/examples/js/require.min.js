...
Q.test(a[l]) && (a[l] = a[l].replace(Q, '')); a[0].charAt(0) === '.' && k && (l = k.slice(0, k.length - 1), a = l.concat(a)); l = a; for (c = 0; c < l.length; c++) if (d = l[c], d === '.')l.splice(c, 1), c -= 1; else if (d === '..' && !(c === 0 || c == 1 && l[2] === '..' || l[c - 1] === '..') && c > 0)l.splice(c - 1, 2), c -= 2; a = a.join('/');
      ...
      k, b) {
      let f = a.id,
        c = m(h, f); if (t(r, f) && (!c || c.defineEmitComplete))k === 'defined' && b(r[f]); else if (c = s(a), c.error && k === 'error')b(c.error); else c.on(k, b);
    } function w(a, b) {
      let c = a.requireModules,
        f = !1; if (b)b(a); else if (v(c, function(b) { if (b = m(h, b))b.error = a, b.events.error && (f = !0, b.emit('error', a)); }), !f)g.onError(a);
    } function x() { R.length && (ha.apply(A, [ A.length, 0 ].concat(R)), R = []); } function y(a) { delete h[a]; delete V[a]; } function F(a, b, c) {
      const f = a.map.id; a.error ? a.emit('error', a.error) : (b[f] = !0, v(a.depMaps, function(f,
        d) {
        const e = f.id,
    ...
    } function E(a) { t(r, a[0]) || s(p(a[0], null, !0)).init(a[1], a[2]); } function I(a) {
      ...
      aa = {},
      K = 1,
      O = 1; L = { require(a) { return a.require ? a.require : a.require = i.makeRequire(a.map); }, exports(a) { a.usingExports = !0; if (a.map.isDefine) return a.exports ? r[a.map.id] = a.exports : a.exports = r[a.map.id] = {}; }, module(a) {
      return a.module ?
...
c; this.inited = !0; this.ignore = f.ignore; f.enabled || this.enabled ? this.enable() : this.check();
    ...
    }, defineDep(a, b) { this.depMatched[a] || (this.depMatched[a] = !0, this.depCount -= 1, this.depExports[a] = b); }, fetch() { if (!this.fetched) { this.fetched = !0; i.startTime = (new Date()).getTime(); const a = this.map; if (this.shim)i.makeRequire(this.map, { enableBuildCallback: !0 })(this.shim.deps || [], u(this, function() { return a.prefix ? this.callPlugin() : this.load(); })); else return a.prefix ? this.callPlugin() : this.load(); } }, load() {
      const a =
...
this.map,
        ...
        let l,
          d; d = m(aa, this.map.id); let e = this.map.name,
          ...
          d ? (this.map.url = i.nameToUrl(d), this.load()) : (l = u(this, function(a) { this.init([], function() { return a; }, null, { enabled: !0 }); }), l.error = u(this, function(a) { this.inited = !0; this.error = a; a.requireModules = [ b ]; B(h, function(a) { a.map.id.indexOf(b + '_unnormalized') === 0 && y(a.map.id); }); w(a); }), l.fromText = u(this, function(f, c) {
            const d = a.name,
              e = p(d),
              P = M; c && (f = c); P && (M = !1); s(e); t(j.config, b) && (j.config[d] = j.config[b]); try { g.exec(f); } catch (h) {
    ...
    }, on(a, b) { let c = this.events[a]; c || (c = this.events[a] = []); c.push(b); }, emit(a, b) { v(this.events[a], function(a) { a(b); }); a === 'error' && delete this.events[a]; } }; i = { config: j, contextName: b, registry: h, defined: r, urlFetched: S, defQueue: A, Module: Z, makeModuleMap: p,
      nextTick: g.nextTick, onError: w, configure(a) {
        a.baseUrl && a.baseUrl.charAt(a.baseUrl.length - 1) !== '/' && (a.baseUrl += '/'); const b = j.shim,
          c = { paths: !0, bundles: !0, config: !0, map: !0 }; B(a, function(a, b) { c[b] ? (j[b] || (j[b] = {}), U(j[b], a, !0, !0)) : j[b] = a; }); a.bundles && B(a.bundles, function(a, b) { v(a, function(a) { a !== b && (aa[a] = b); }); }); a.shim && (B(a.shim, function(a, c) { H(a) && (a = { deps: a }); if ((a.exports || a.init) && !a.exportsFn)a.exportsFn = i.makeShimExports(a); b[c] = a; }), j.shim = b); a.packages && v(a.packages, function(a) {
          var b,
      ...
      }, makeShimExports(a) { return function() { let b; a.init && (b = a.init.apply(ba, arguments)); return b || a.exports && da(a.exports); }; }, makeRequire(a, e) {
        function j(c, d, m) {
          let n,
            q; e.enableBuildCallback && (d && G(d)) && (d.__requireJsBuild =
!0); if (typeof c === 'string') { if (G(d)) return w(C('requireargs', 'Invalid require call'), m); if (a && t(L, c)) return L[c](h[a.id]); if (g.get) return g.get(i, c, a, j); n = p(c, a, !1, !0); n = n.id; return !t(r, n) ? w(C('notloaded', 'Module name "' + n + '" has not been loaded yet for context: ' + b + (a ? '' : '. Use require([])'))) : r[n]; }J(); i.nextTick(function() { J(); q = s(p(null, a)); q.skipMap = e.skipMap; q.init(c, d, m, { enabled: !0 }); D(); }); return j;
        }e = e || {}; U(j, { isBrowser: z, toUrl(b) {
          let d,
            e = b.lastIndexOf('.'),
            k = b.split('/')[0]; if (e !==
-1 && (!(k === '.' || k === '..') || e > 1))d = b.substring(e, b.length), b = b.substring(0, e); return i.nameToUrl(c(b, a && a.id, !0), d, !0);
        }, defined(b) { return t(r, p(b, a, !1, !0).id); }, specified(b) { b = p(b, a, !1, !0).id; return t(r, b) || t(h, b); } }); a || (j.undef = function(b) {
          x(); const c = p(b, a, !0),
            e = m(h, b); d(b); delete r[b]; delete S[c.url]; delete $[b]; T(A, function(a, c) { a[0] === b && A.splice(c, 1); }); e && (e.events.defined && ($[b] = e.events), y(b));
        }); return j;
      }, enable(a) { m(h, a.id) && s(a).enable(); }, completeLoad(a) {
        let b,
          c,
          d = m(j.shim, a) || {},
          g = d.exports; for (x(); A.length;) { c = A.shift(); if (c[0] === null) { c[0] = a; if (b) break; b = !0; } else c[0] === a && (b = !0); E(c); }c = m(h, a); if (!b && !t(r, a) && c && !c.inited) { if (j.enforceDefine && (!g || !da(g))) return e(a) ? void 0 : w(C('nodefine', 'No define call for ' + a, null, [ a ])); E([ a, d.deps || [], d.exportsFn ]); }D();
      }, nameToUrl(a, b, c) {
        let d,
          e,
          h; (d = m(j.pkgs, a)) && (a = d); if (d = m(aa, a)) return i.nameToUrl(d, b, c); if (g.jsExtRegExp.test(a))d = a + (b || ''); else {
      ...
      }, load(a, b) { g.load(i, a, b); }, execCb(a, b, c, d) { return b.apply(d, c); }, onScriptLoad(a) { if (a.type === 'load' || ja.test((a.currentTarget || a.srcElement).readyState))N = null, a = I(a), i.completeLoad(a.id); }, onScriptError(a) {
        const b = I(a); if (!e(b.id)) {
    ...
    z = !!(typeof window !== 'undefined' && typeof navigator !== 'undefined' && window.document),
    ea = !z && typeof importScripts !== 'undefined',
    ja = z && navigator.platform === 'PLAYSTATION 3' ? /^complete$/ : /^(complete|loaded)$/,
    Y = typeof opera !== 'undefined' && opera.toString() === '[object Opera]',
    F = {},
    q = {},
    R = [],
    M = !1; if (typeof define === 'undefined') {
    if (typeof requirejs !== 'undefined') { if (G(requirejs)) return; q = requirejs; requirejs = void 0; } typeof require !== 'undefined' && !G(require) && (q = require, require = void 0); g = requirejs = function(b, c, d, e) {
      let n,
        p = '_'; !H(b) && typeof b !== 'string' && (n = b, H(c) ? (b = c, c = d, d = e) : b = []); n && n.context && (p = n.context); (e = m(F, p)) || (e = F[p] = g.s.newContext(p)); n && e.configure(n); return e.require(b, c, d);
    }; g.config = function(b) { return g(b); };
    g.nextTick = typeof setTimeout !== 'undefined' ? function(b) { setTimeout(b, 4); } : function(b) { b(); }; require || (require = g); g.version = '2.1.15'; g.jsExtRegExp = /^\/|:|\?|\.js$/; g.isBrowser = z; x = g.s = { contexts: F, newContext: ga }; g({}); v([ 'toUrl', 'undef', 'defined', 'specified' ], function(b) { g[b] = function() { const c = F._; return c.require[b].apply(c, arguments); }; }); if (z && (y = x.head = document.getElementsByTagName('head')[0], D = document.getElementsByTagName('base')[0]))y = x.head = D.parentNode; g.onError = ca; g.createNode = function(b) {
      const c =
...
O), s = s.replace(Q, ''), g.jsExtRegExp.test(s) && (s = I), q.deps = q.deps ? q.deps.concat(s) : [ s ], !0;
      ...
      let e,
        g; typeof b !== 'string' && (d = c, c = b, b = null); H(c) || (d = c, c = null); !c && G(d) && (c = [], d.length && (d.toString().replace(ka, '').replace(la, function(b, d) { c.push(d); }), c = (d.length === 1 ? [ 'require' ] : [ 'require', 'exports', 'module' ]).concat(c))); if (M) {
